...
export default (bot: Eris.Client): Command => ({
    name: 'resource_edit_modal',
    description: 'Handle editing resource modal submission',
    ...
    async execute(interaction: Eris.Interaction): Promise<void> {
        ...
        try {
            ...
            resource = await databaseManager.getResource(resourceId);

            await bot.editMessage(message.channel.id, message.id, {
                embeds: [{
                    ...message.embeds[0],
                    description: [ `<:${emojiName}:${emojiId}> **Resource Title:** \`${resource.title}\``,
                    `<:${emojiName}:${emojiId}> **Resource Tag:** \`${resource.tag}\``,
                    `<:${emojiName}:${emojiId}> **Submitted by:** <@${resource.author}> | \`${(await bot.users.get(resource.author))?.username}\``,
                    `<:${emojiName}:${emojiId}> **Resource Link:** \`${resource.url}\``,
                    resource.description ? `<:blue:${blue}> **Description:** \`${resource.description}\`` : ''
                    ].join('\n')
                }],
