...
export default (bot: Eris.Client): Command => ({
    name: 'notifier_botupdates',
    description: 'Add/Remove the bot updates notifier role',
    ...
    async execute(interaction: Eris.Interaction): Promise<void> {
        ...
        const roleID = roles.botUpdates;
        if (!roleID) return;
        if (member.roles.includes(roleID)) {
            try {
                await member.removeRole(roleID, 'Notifier role removed via button');
                await interaction.createMessage({
                    content: `- From now on, you will **__not__** be pinged whenever **bot changes** are pushed!`,
                    flags: 64
                });
            } catch (error) {
            ...
            try {
                ...
                await interaction.createMessage({
                    content: `- From now on, you will be pinged whenever **bot changes** are pushed!`,
                    flags: 64
                });
            } catch (error) {
